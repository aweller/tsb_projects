...
def fetch_bam(sample, bam_location):
    ...
    out_dir = "/home/ionadmin/andreas/projects/ionoxford/TSB_Tools/fetch_bams/scripts/" 
    out_file = "%s.zip" % sample
    out = out_dir + out_file
    
    curl_cmd = """curl --request POST -d "name=%s" --header "Authorization:%s" https://ir16.ionreporter.iontorrent.com/rest/api/download?name -o %s""" % (bam_location, token, out)
